...
const Card: FC<CardProps> = ({ card, ...props }) => {
  ...
  return (
    <Div
      {...props}
      className="m-2 border-2 border-b rounded-2xl overflow-hidden shadow-2xl"
    >
      <div className="flex flex-col">
        <Div src={image} className="h-60" />
        <Div className="p-4" height="16rem" minHeight="21rem">
          ...
          <p className="mt-2 line-clamp-4 overflow-hidden h-30">{paragraphs}</p>
          <Div className="flex flex-row items-center justify-between p-2 mt-2 text-red-500 relative bottom-0">
            {icons}
          </Div>
        </Div>
      </div>
